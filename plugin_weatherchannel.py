...
def poll(sensor):
	...
	if request == "alerts":
		...
		return utils.web_get(url+str(plugin_conf['latitude'])+'/'+str(plugin_conf['longitude'])+'/'+get_request_type(sensor['plugin']['measure'])+'/wwir.json?apiKey='+plugin_conf['api_key']+'&units=m&language=en')
